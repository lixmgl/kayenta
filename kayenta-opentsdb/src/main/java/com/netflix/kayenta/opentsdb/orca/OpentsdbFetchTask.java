...
@Component
@Slf4j
public class OpentsdbFetchTask implements RetryableTask {
  private final ObjectMapper kayentaObjectMapper;
  private final AccountCredentialsRepository accountCredentialsRepository;
  private final SynchronousQueryProcessor synchronousQueryProcessor;

  @Autowired
  public OpentsdbFetchTask(ObjectMapper kayentaObjectMapper,
  ...
  public TaskResult execute(@Nonnull Stage stage) {
    ...
    OpentsdbCanaryScope canaryScope;

    try {
      canaryScope = kayentaObjectMapper.readValue((String)stage.getContext().get("canaryScope"), OpentsdbCanaryScope.class);
    } catch (IOException e) {
